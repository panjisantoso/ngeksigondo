            ...
            <table class="table table align-items-center table-flush" id="laravel_crud" >
              ...
              <tbody class="list">
                @for($i = 1; $i <= sizeof($kecamatan); $i++)
                  <tr id="row_{{$kecamatan[$i-1]->id}}">
                    <td>
                    {{ $i }}
                    </td>
                    <td>
                    {{ $kecamatan[$i-1]->nama_kecamatan }}
                    </td>
                    <td>
                    {{ $kecamatan[$i-1]->nama_kabupaten }}
                    </td>
                    <td>
                      <a href="javascript:void(0)" data-id="{{ $kecamatan[$i-1]->id }}" onclick="editKabupaten(event.target)" class="btn btn-info">Edit</a>
                      <a href="javascript:void(0)" data-id="{{ $kecamatan[$i-1]->id }}" class="btn btn-danger" onclick="deleteKabupaten(event.target)">Delete</a>
                    </td>
                  </tr>
                @endfor
              </tbody>
